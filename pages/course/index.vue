              ...
              <dd class="c-s-dl-li">
                <ul class="clearfix">
                  <li v-for="(twoSubject,index) in subSubjectList" :key="index" :class="{active:twoIndex==index}">
                    <a :title="twoSubject.title" href="#" @click="searchTwo(twoSubject.id,index)">{{twoSubject.title}}</a>
                  </li>
                </ul>
              </dd>
            </dl>
...
<script>
    ...
    export default {
        ...
        methods:{
            // 查询课程列表
            initCourse(){
                courseApi.getPageCourseWithCondition(1,8,this.searchObj)
                    .then(response => {
            ...
            gotoPage(page){
                ...
                this.page = page
                courseApi.getPageCourseWithCondition(page,8,this.searchObj)
                    .then(response => {
                        this.data = response.data.data.map
                    })
            },

            // 点击一级分类，显示一级分类下的二级分类，并对课程列表进行条件查询
